...
class UsersIndexAdminTest < UsersIndexAdmin
  ...
  test "should display only activated users" do
    # Deactivate thi first user on the page.
    # Making an inactive user fixture is not
    # sufficient because Rails can't guarantee
    # it would appear on the first page.
    User.paginate(page: 1).first.toggle!(:activated)
    # Ensure that all the displayed users are
    # activated.
    # assigns(:users) is taking @users variable
    # coming form the controller, that will be
    # used to fill the view
